...
//commentId= 'id="comment' + num;
     input.submit function postComment(commentId){
    if(input.placeholder=="Add a reply"){
        document.getElementById(commentId).outerHTML+=fourth + input.value + fifth;
      }
      else{
        num++;
        input.value;
        container.innerHTML += first +commentId+'"'+second + input.value + third;
      }
      input.value="";
    }
